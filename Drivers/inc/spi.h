...
#define SPI_SR_FLAG_RXNE 							((uint16_t)0x0001)
#define SPI_SR_FLAG_TXE 							((uint16_t)0x0002)
#define SPI_SR_FLAG_BSY 							((uint16_t)0x0080)
...
typedef struct
{
	uint16_t mode;		// Master/Slave
	uint16_t baud_rate;	// Baudrate prescaler
	uint16_t ss;		// Internal NSS state
	uint16_t cpol;		// Clock polarity
	uint16_t cpha;		// Clock phase
	uint16_t first_bit;	// MSB or LSB
	uint16_t data_size;	// 8-bit or 16-bit
	uint16_t direction; // Full duplex / RX only
} SPI_InitTypeDef;
